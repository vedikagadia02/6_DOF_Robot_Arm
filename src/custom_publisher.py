...
theta1, theta2, theta3, theta4, theta5, theta6 = -pi, pi/2, 0, 0, 0, 0
radius = 0.4
duration = 5
num_steps = 100
omega = (2 *pi)/duration
...
def circleTrajectory(r, x, y, z):
    radius = r
    theta_values = np.linspace(np.pi/2, np.pi/2 + 2*np.pi, num_steps)
...
def move_to_points():
    ...
    while (not rospy.is_shutdown()):
        for i in range(len(joint_angles[1])):
            ...
            pub_joint1_pos.publish(q1+pi) #  To match DH calculation to gazebo world 
            rate.sleep()
            pub_joint2_pos.publish(q2-pi/2)
            rate.sleep()
...
if __name__ == '__main__':
    try:
        theta_values, x_values, y_values, z_values = circleTrajectory(radius, 2.675, 0, 4.30)
        ...
        for i in range(num_steps):
            theta = theta_values[i]
            vx_values[i] = 0
            vy_values[i] = -radius * np.sin(theta) * omega
            vz_values[i] = radius * np.cos(theta) * omega

        joint_velocities = np.zeros((6, len(vx_values)))
        joint_angles = np.zeros((6, len(vx_values)))
        joint_angles[:,0] = np.array([theta1, theta2, theta3, theta4, theta5, theta6])
